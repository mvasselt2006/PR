...
def create_label(sample_size):
  labels =torch.randint(low=0,high=9,size=sample_size).to(device)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            shaped_vector = torch.reshape(label_vector.detach().clone(),[128,10,1,1])
            ...
            data_real = image
            loss_d += train_discriminator(optim_d, data_real, data_fake,label_vector)

            #generate input noise and label for training generator
            input_noise=create_noise(b_size, nz)
            label_vector= create_label(b_size)
            shape_vector= torch.reshape(label_vector.detach().clone(),[b_size,10,1,1])
            label_noise= torch.cat((input_noise,shaped_vector),1) #Creates [128,110,1,1] noise vector

            data_fake = generator(label_noise)
            loss_g += train_generator(optim_g, data_fake,label_vector)
